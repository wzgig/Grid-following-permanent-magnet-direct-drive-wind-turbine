...
%% ========== xi求解 ==========
eqnq = Ki_PLL*(v_g_q/Vbase - ugq_ref) == 0;          % state16
[pllg] = solve(eqnq, thetapll);     % 这里不需要 eval([eqnq])，eqnq 本来就是符号方程

% ====== 1) 取所有解的实部（如果是复数，只保留实部）======
sols = vpa(pllg);
sols = real(sols);

% ====== 2) 把解按 2π 周期平移，使其尽量靠近 xi ======
% 对每个解：sol_adj = sol + 2*pi*round((xi - sol)/(2*pi))
sols_adj = sols + 2*pi*round((xi - sols)/(2*pi));

% ====== 3) 约束范围：thetapll ∈ [xi-0.05, xi+0.05] ======
lb = xi - 0.05;
ub = xi + 0.05;

% 先找是否有解落在范围内
idx_in = find(sols_adj >= lb & sols_adj <= ub);

if ~isempty(idx_in)
    % 若有多个，选最接近 xi 的那个
    [~, k] = min(abs(sols_adj(idx_in) - xi));
    thetapll = sols_adj(idx_in(k));
else
    % 若没有任何解落在范围内：取最接近 xi 的解，然后 clamp 到范围内
    [~, k] = min(abs(sols_adj - xi));
    thetapll = sols_adj(k);
    thetapll = min(max(thetapll, lb), ub);   % clamp
end
disp(thetapll)
PLL_int = 0;
...
[t,x] = ode45(@(t,x) tuoMW(t,x,omega_best, i_gqref),tspan,double(inistate),options);
